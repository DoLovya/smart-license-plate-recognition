...
## 0.4.6 / 2026-07-11 — Qt 客户端新增三套主题与样式切换

### 变更

- 为 Qt 客户端新增三套可切换 QSS 主题：`A 工业深色蓝灰`、`B 极简黑灰青`、`C 浅色企业版`。
- 在主界面头部新增主题切换控件，并支持运行时切换应用级样式。
- 主题选择结果写入 `QSettings`，客户端下次启动时会自动恢复上次选中的样式。
- 将样式资源拆分为独立 QSS 文件并纳入 `resources.qrc` 管理。

## 0.4.5 / 2026-07-11 — Qt 主界面预览区比例固定

### 变更

- 调整 Qt 客户端主界面左右区域布局：在窗口缩放时锁定预览区与结果区的宽度比例，避免左侧预览区随内容观感漂移。
- 下调预览容器与图片控件的最小尺寸约束，减少大尺寸最小宽度对整体布局的挤压。
- `ImagePreviewWidget` 改为完整显示优先：由 `KeepAspectRatioByExpanding` 切换为 `KeepAspectRatio`，导入图片后默认先完整展示，不再裁切填满。
